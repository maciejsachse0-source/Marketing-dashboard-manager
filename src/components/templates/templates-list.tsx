...
export function TemplatesList({ templates }: { templates: ProductionTemplate[] }) {
  ...
  return (
    <div className="space-y-10">
      {withArtist.length > 0 ? (
        <section>
          <ListSectionHeading title="Z artystą" count={withArtist.length} />
      ...
      {solo.length > 0 ? (
        <section>
          <ListSectionHeading title="Solo" count={solo.length} />
          <div className="grid lg:grid-cols-2 gap-5">
            {solo.map((t) => (
              <TemplateCard key={t.slug} template={t} />
            ))}
